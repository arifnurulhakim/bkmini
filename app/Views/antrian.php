                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text bg-warning text-white">noMeja</span>
                                </div>
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input type="number" id="nominal" class="form-control" disabled aria-label="Amount (to the nearest dollar)">
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text bg-warning text-white">Total Rp.</span>
                                </div>
                    ...
                    <div class="col-6">
                        <div class="form-group" id="">
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text bg-warning text-white">Kembalian Rp.</span>
                                </div>
                                <input type="number" id="kembalian" class="form-control" disabled aria-label="Amount (to the nearest dollar)" disabled>
                            </div>
                        </div>
                    </div>
                    <div class="col-3"></div>
...
<script>
            ...
            success: function(data) {
                if (data.length) {
                    for (let i = 0; i < data.length; i++) {
                        isiPesanan += "<tr><td>" + data[i].noMeja + "</td><td>" + data[i].nama + "</td><td>" + data[i].nominal + "</td><td>"
                        ...
                        isiPesanan += "</label></td><td><button href='#'  class='btn btn-inverse-warning btn-sm' onClick='modalRincian(" + data[i].id + ", \" " + data[i].nama + " \"," + data[i].noMeja + "," + data[i].nominal + "," + data[i].status + " )'><i class='mdi mdi-format-list-bulleted-type'></i><i class='mdi mdi-food-fork-drink'></i></button></td></tr>"
                    }
                } else {
    ...
    function tampilkanAntrianSelesai() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.length) {
                    for (let i = 0; i < data.length; i++) {
                        isiPesanan += "<tr><td>" + data[i].noMeja + "</td><td>" + data[i].nama + "</td><td>" + data[i].nominal + "</td><td><label class='badge badge-success'>Selsai :)</label></td><td><button href='#' class='btn btn-inverse-success btn-sm' onClick='modalRincian(" + data[i].id + ", \" " + data[i].nama + " \"," + data[i].noMeja + "," + data[i].nominal + "," + data[i].status + " )'><i class='mdi mdi-playlist-check'></i><i class='mdi mdi-food'></i></button></td></tr>"
                    }
                } else {
    ...
    function modalRincian(id, nama, noMeja, nominal, status) {
        $("#nama").val(nama)
        $("#noMeja").val(noMeja)
        $("#nominal").val(nominal)
        $("#proses").show()

        tampilkanRincian(id, nominal)
        if (status == 0) {
            $("#proses").html("Bayar")
        } else if (status == 1) {
            $("#proses").html("Selesai")
    ...
    function tampilkanRincian(id, nominal) {
        var isiPesanan = ""
        var totalHarga = 0
        var nominal = nominal
        var kembalian = 0
        $.ajax({
            ...
            success: function(data) {
                ...
                // if (data2.length) {
                //     for (let i = 0; u < data2.length; u++) {
                //         kembalian += data2[u].nominal - totalHarga
                //     }
                // } 
                else {
                    isiPesanan = "<td colspan='4'>Antrian Masih Kosong :)</td>"
        ...
        $.ajax({
            url: '<?= base_url() ?> /antrian/rincianKembalian',
            method: 'post',
            data2: "nominal=" + id,
            dataType: 'json',
            success: function(data) {
